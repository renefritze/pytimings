...
def csv_to_dataframe(filenames: Iterable[Union[str, Path]], sort=False):
    ...
    assert all(
        ...
        == dataframe["pytimings::data::_sections"].iloc[0]
    )
    if not all(
        dataframe["pytimings::data::_version"]
        == dataframe["pytimings::data::_version"].iloc[0]
    ):
        getLogger("").warning(
            "input csv files created from different pytimings versions"
        )
